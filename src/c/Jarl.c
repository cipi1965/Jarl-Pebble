...
static void bluetooth_callback(bool connected) {
 ...
 // layer_set_hidden(bitmap_layer_get_layer(s_bt_icon_layer), connected);
  if(!connected) {
    ...
    if (settings.VibrateOnDisconnect) vibes_double_pulse();
  } else {
    if (settings.VibrateOnConnect) vibes_short_pulse();
  }
}
...
static void update_time_component(char *current_char, char buffer, int *color, int number, BitmapLayer* layer, GBitmap* bitmap) {
#ifdef PBL_COLOR
    if ((*current_char != buffer) | (start == true)) {
      *color = rand()%4;
    }
    gbitmap_destroy(bitmap);
    bitmap = gbitmap_create_with_resource(NUMBERS_IDS[number]);
    replace_gbitmap_color(GColorWhite, settings.Colors[*color], bitmap, NULL);
...
static void update_time() {
  ...
  int h1 = buffer[0]  - '0';
  update_time_component(&hours_dec, buffer[0], &color1, h1, hour_layer_dec, hour_bitmap_dec);

  int h0 = buffer[1]  - '0';
  update_time_component(&hours_unit, buffer[1], &color2, h0, hour_layer_unit, hour_bitmap_unit);
  
  int m1 = buffer[3]  - '0';
  update_time_component(&minutes_dec, buffer[3], &color3, m1, minute_layer_dec, minute_bitmap_dec);
  
  int m0 = buffer[4]  - '0';
  update_time_component(&minutes_unit, buffer[4], &color4, m0, minute_layer_unit, minute_bitmap_unit);
...
static void in_received_handler(DictionaryIterator *iter, void *context) 
{
  ...
  if (temperature_t) {
    ...
    gbitmap_destroy(bitmap_temp_sign);
    if(temperature_buffer[0] == '+') {bitmap_temp_sign = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_PLUS_MINI);}
    if(temperature_buffer[0] == '-') {bitmap_temp_sign = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_MINUS_MINI);}
    bitmap_layer_set_bitmap(layer_temp_sign, bitmap_temp_sign);
  ...
  if (conditions_t) {
    ...
    gbitmap_destroy(conditions_bitmap);
    if (condition == 0) {conditions_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_W_CLEAR);}
    if (condition == 1) {conditions_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_W_CLEAR_NIGHT);}
    if (condition == 2) {conditions_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_W_PARTLY_CLOUDY);}
    if (condition == 3) {conditions_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_W_PARTLY_CLOUDY_NIGHT);}
    if (condition == 4) {conditions_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_W_CLOUDY);}
    if (condition == 5) {conditions_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_W_FOG);}
    if (condition == 6) {conditions_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_W_DRIZZLE);}  
    if (condition == 7) {conditions_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_W_RAIN);}
    if (condition == 8) {conditions_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_W_SNOW);}              
    if (condition == 9) {conditions_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_W_THUNDER);}
